  ...
  const showToast = (notif: AppNotification) => {
    ...
    if (typeof window !== 'undefined' && 'Notification' in window) {
      if (Notification.permission === 'granted') {
        new Notification("SAT Tracker", { body: notif.message.replace(/{{planId:.*?}}/g, "").trim() });
  ...
  useEffect(() => {
    ...
    async function checkAndCreate() {
        ...
        for (const plan of plans) {
            ...
            // Schedule Local Notification (Mobile/Background)
            if (Platform.OS !== 'web') {
            ...
            // Notify 1 min before start OR if late but before end
