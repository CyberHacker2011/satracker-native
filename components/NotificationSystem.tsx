...
import { Bell, X, Play, Info, AlertCircle } from "lucide-react-native";
          ...
          if (!playedSoundsRef.current.has(tomKey)) {
            ...
            if (count === 0) {
              ...
              await supabase.from("notifications").insert({
                user_id: user.id,
                message:
                  "You haven't created a plan for tomorrow. Stay organized to succeed! {{goToPlan:true}} {{type:tomorrow_reminder}}",
                created_at: new Date().toISOString(),
              });
            } else {
              playedSoundsRef.current.add(tomKey);
    ...
    async function checkNotifications() {
      ...
      if (data && data[0] && mounted) {
        ...
        if (!playedSoundsRef.current.has(notif.id)) {
          const isNew =
            new Date().getTime() - new Date(notif.created_at).getTime() < 60000;
    ...
    >
      <Card style={styles.toastCard}>
        <View style={styles.header}>
          ...
          >
            {isMissed ? (
              <AlertCircle size={12} color={theme.primary} />
            ) : (
              <Bell size={12} color={theme.primary} />
            )}
            <ThemedText style={[styles.badgeText, { color: theme.primary }]}>
              {isMissed ? "MISSED MISSION" : "NEW ALERT"}
            </ThemedText>
          </View>
        ...
        {planId && (
          <TouchableOpacity
            ...
            onPress={async () => {
              ...
              const isPast = planData
                ? new Date() > new Date(planData.date + "T" + planData.end_time)
                : false;
          ...
          >
            ...
            <ThemedText style={styles.actionText}>
              {isMissed ? "Check-in Now" : "Enter Study Room"}
            </ThemedText>
          </TouchableOpacity>
        )}
      </Card>
