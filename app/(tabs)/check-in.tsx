...
export default function CheckInScreen() {
  ...
  const checkPlanTimeStatus = (
    ...
    const isPast =
      dateStr < todayStr || (dateStr === todayStr && curTime > endTime);
    const isUpcoming =
      dateStr > todayStr || (dateStr === todayStr && curTime < startTime);
    const isActive =
      dateStr === todayStr && curTime >= startTime && curTime <= endTime;
    ...
    try {
      ...
      if (plansData) {
        plansData.forEach((p: any) => {
          ...
          const { isPast, isUpcoming, isActive } = checkPlanTimeStatus(
            p.date,
            p.start_time,
            p.end_time,
          );
  ...
  const handleCheckIn = async (plan: StudyPlan, status: Status) => {
    ...
    try {
      ...
      if (existing) {
        ...
        const { error: err } = await supabase.from("daily_log").insert({
          user_id: user.id,
          plan_id: plan.id,
          date: plan.date,
          status,
      ...
      const isOnline = await checkConnection();
      setErrorMsg(
        isOnline ? e.message || t("failedSaveStatus") : t("noInternet"),
      );
    } finally {
      setSavingId(null);
    }
  ...
  const handleDeletePlan = async (id: string) => {
    ...
    if (Platform.OS === "web") {
      ...
      Alert.alert(t("deletePlan"), t("sureDelete"), [
            ...
            <ScrollView contentContainerStyle={styles.container}>
              <View style={styles.header}>
                ...
                >
                  ...
                  <ThemedText
                    style={{ fontSize: 12, fontWeight: "800", opacity: 0.5 }}
                  >
                ...
                >
                  {dayLabels.map((label) => (
                    <View key={label} style={styles.calLabelCell}>
                      <ThemedText style={styles.calLabelText}>
                        {label}
                      </ThemedText>
                    </View>
                  ))}
                ...
                <View style={styles.calGrid}>
                  {calendarDays.map((day, idx) => {
                    ...
                    const hasIncomplete = plans.some(
                      (p) => !p.status && !p.isPast && !p.isUpcoming,
                    );
                          ...
                          {hasPlans && (
                            <View
                              style={[
                                styles.planDot,
                                {
                                  backgroundColor: plans.some(
                                    (p) => p.status === "done",
                                  )
                                    ? "#10b981"
                                    : plans.some((p) => p.status === "missed")
                                      ? "#ef4444"
              ...
              <View style={styles.legend}>
                <View style={styles.legendItem}>
                  <View
                    style={[styles.planDot, { backgroundColor: theme.primary }]}
                  />
                ...
                <View style={styles.legendItem}>
                  <View
                    style={[styles.planDot, { backgroundColor: "#10b981" }]}
                  />
                  <ThemedText style={styles.legendText}>
                    {t("finished")}
                  </ThemedText>
            ...
            <View style={styles.modalOverlay}>
              <Card style={[styles.modalCard, { backgroundColor: theme.card }]}>
                ...
                <ScrollView style={styles.modalScroll}>
                  ...
                  plansByDate[selectedDate].length === 0 ? (
