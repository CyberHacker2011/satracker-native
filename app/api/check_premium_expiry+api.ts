...
async function sendEmail(userEmail: string, subject: string, message: string) {
  ...
  try {
    ...
    await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || "notifications@satracker.uz",
      to: userEmail,
      subject,
  ...
  try {
    ...
    for (const user of (users || [])) {
        ...
        if (expiryDate < now) {
            ...
            const message = "Your Premium subscription has expired. Renew now to continue enjoying unlimited features.";
            ...
            if (user.email) {
                const sent = await sendEmail(user.email, "Premium Expired", message);
                if (sent) emailsSent++;
            }
        } 
        // 2. Check for warning (expires within 24 hours)
            ...
            const warningMessage = "Your Premium subscription will expire in less than 24 hours.";
            ...
            if (!existing) {
                ...
                if (user.email) {
                    const sent = await sendEmail(user.email, "Premium Expiring Soon", warningMessage);
                    if (sent) emailsSent++;
                }
            }
        }
