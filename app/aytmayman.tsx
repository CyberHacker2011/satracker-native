...
export default function AdminPremiumPage() {
  ...
  const grantPremium = async (userId: string, userName: string, months: 1 | 3) => {
      ...
      <ThemedView style={{ flex: 1 }}>
        <SafeAreaView style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 }}>
          <View style={[styles.loginCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            <ThemedText style={styles.loginSubtitle}>Enter admin password</ThemedText>
            ...
            <TouchableOpacity
              ...
              onPress={handleLogin}
            >
              <ThemedText style={styles.loginButtonText}>Access Admin Panel</ThemedText>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      </ThemedView>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.container}>
          <View style={styles.header}>
            ...
            <ThemedText style={styles.subtitle}>Grant premium subscriptions to users</ThemedText>
          </View>

          {error ? (
            <Card style={styles.errorCard}>
              <ThemedText style={{ color: '#ef4444', fontWeight: '700' }}>{error}</ThemedText>
              <TouchableOpacity 
                style={[styles.retryButton, { backgroundColor: theme.primary }]}
                onPress={loadUsers}
              >
                <ThemedText style={{ color: '#fff', fontWeight: '900' }}>Retry</ThemedText>
              </TouchableOpacity>
            </Card>
          ) : null}
          ...
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <ThemedText style={{ marginTop: 12, opacity: 0.5 }}>Loading users...</ThemedText>
            </View>
          ) : (
                  ...
                  <Card key={user.id} style={styles.userCard}>
                        ...
                        {user.is_premium && user.premium_expires_at ? (
                          <View style={styles.premiumBadge}>
                            ...
                            <ThemedText style={styles.premiumText}>
                              Premium until {new Date(user.premium_expires_at).toLocaleDateString()}
                            </ThemedText>
                          </View>
                        ) : null}
                      </View>
                    ...
                    <View style={styles.actions}>
                      <TouchableOpacity
                        ...
                        onPress={() => grantPremium(user.id, user.name || user.email, 1)}
                      >
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => grantPremium(user.id, user.name || user.email, 3)}
                      >
